...
def anonymize(maps: dict, folder: str, home: str=DATA) -> None:
    previous_address = home / folder
    new_address = home / maps[folder]
    shutil.move(previous_address, new_address)
    print(f"Data previously in folder {folder[:3]} now loacated in {new_address}")

def main():
    print(f"Current location: {Path(__file__)}")
    maps = load_yaml(DATA / "anonymise.yaml")
